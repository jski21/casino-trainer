...
const getCountLabel = (value) => {
  ...
  if (v > 0) return { label:"+1", color:"var(--accent)" };
  if (v < 0) return { label:"-1", color:"var(--danger)" };
...
export default function BlackjackTrainer() {
  ...
  const renderChart = (rows) => (
    <div style={{ overflowX:"auto", WebkitOverflowScrolling:"touch", width:"100%", maxWidth:700, margin:"0 auto" }}>
      <table style={{ borderCollapse:"collapse", fontSize:13, minWidth:310, width:"100%" }}>
        <thead>
          <tr>
            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"left", fontWeight:700, fontSize:12, letterSpacing:"0.1em" }}>HAND</th>
              ...
              {row.actions.map((a,ai)=>{
                ...
                return (
                  <td key={ai} style={{ padding:"5px 8px", textAlign:"center" }}>
                    <div style={{
                      background:st.bg, border:`1px solid color-mix(in srgb, ${st.border} 27%, transparent)`, borderRadius:3,
  ...
  return (
    <div style={{
      ...
      fontFamily:"'Courier New', monospace", color:"var(--text)",
      ...
      <div style={{ textAlign:"center", marginBottom:8 }}>
        <div style={{ fontSize:"clamp(9px,1.2vw,14px)", letterSpacing:"0.5em", color:"var(--accent)", marginBottom:4 }}>HI-LO · BASIC STRATEGY · DEVIATIONS</div>
        <h1 style={{ fontSize:"clamp(22px,3.8vw,52px)", fontWeight:900, margin:0, letterSpacing:"-0.02em" }}>
          BLACKJACK <span style={{ color:"var(--accent)" }}>TRAINER</span>
        </h1>
      </div>

      {/* Mode tabs */}
      <div style={{ display:"flex", gap:5, marginBottom:12, background:"var(--bg-elev)", borderRadius:8, padding:3 }}>
        {MODES.map(m=>(
      ...
      {mode==="Guide" && (
        <div style={{ width:"100%", maxWidth:"min(820px,95vw)", padding:"8px 4px 48px", display:"flex", flexDirection:"column", gap:24 }}>
          ...
          {/* House Edge */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>HOUSE EDGE</div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[
                { label:"Typical recreational player",   edge:"2 – 3%",   note:"guessing, hunches, side bets",      color:"var(--danger)" },
                { label:"Perfect basic strategy",        edge:"~0.5%",    note:"6-deck, S17, DAS — no counting",    color:"var(--warn)" },
                { label:"Basic strategy + Hi-Lo counting", edge:"−0.5 to −1%", note:"player edge with proper bet spread", color:"var(--accent)" },
              ].map(r=>(
                <div key={r.label} style={{ display:"flex", alignItems:"center", gap:12, background:"var(--bg-elev)", borderRadius:8, padding:"12px 16px" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-3)", marginTop:2 }}>{r.note}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Basic Strategy */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>BASIC STRATEGY</div>
            <div style={{ fontSize:14, color:"var(--text)", lineHeight:1.8, marginBottom:10 }}>
              Basic strategy is the mathematically optimal play for every hand given your cards and the dealer&apos;s upcard. It was derived by computer simulation of billions of hands and eliminates all guesswork. Playing basic strategy perfectly cuts the house edge from 2–3% down to roughly <span style={{ color:"var(--warn)", fontWeight:700 }}>0.5%</span> — the lowest of any casino table game.
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[
                { action:"HIT",    color:"#2a9fd6", desc:"Take another card. Correct when your total is weak and the dealer shows strength (7+)." },
                { action:"STAND",  color:"var(--accent)", desc:"Take no more cards. Correct with stiff totals when the dealer is likely to bust (2–6)." },
                { action:"DOUBLE", color:"var(--warn)", desc:"Double your bet and receive exactly one more card. Best on 10 or 11 vs. weak dealer upcards." },
          ...
          {/* Hi-Lo Card Counting */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>HI-LO CARD COUNTING</div>
            ...
            <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:14 }}>
              {[{cards:"2 – 6",val:"+1",color:"var(--accent)",note:"low cards → count goes up"},{cards:"7 – 9",val:"0",color:"var(--text-3)",note:"neutral"},{cards:"10 – A",val:"−1",color:"var(--danger)",note:"high cards → count goes down"}].map(r=>(
                <div key={r.cards} style={{ background:"var(--bg-elev)", border:`1px solid color-mix(in srgb, ${r.color} 20%, transparent)`, borderRadius:8, padding:"10px 16px", textAlign:"center", flex:1 }}>
                  <div style={{ fontSize:22, fontWeight:900, color:r.color, marginBottom:4 }}>{r.val}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", marginBottom:2 }}>{r.cards}</div>
                  <div style={{ fontSize:11, color:"var(--text-3)" }}>{r.note}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize:14, color:"var(--text)", lineHeight:1.8 }}>
              The <span style={{ color:"var(--warn)", fontWeight:700 }}>running count</span> is the cumulative total as cards are dealt. The <span style={{ color:"var(--warn)", fontWeight:700 }}>true count</span> adjusts for decks remaining:
            </div>
            <div style={{ background:"var(--bg-elev)", border:"1px solid color-mix(in srgb, var(--accent) 20%, transparent)", borderRadius:8, padding:"12px 16px", margin:"10px 0", textAlign:"center", fontSize:16, color:"var(--accent)", fontWeight:700, letterSpacing:"0.05em" }}>
          ...
          {/* Bet Spreading */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>BET SPREADING</div>
            <div style={{ fontSize:14, color:"var(--text)", lineHeight:1.8, marginBottom:10 }}>
              Counting alone doesn&apos;t make you money — you have to vary your bets. Bet small when the count is neutral or negative, and increase your bet when the true count goes positive. The difference between your minimum and maximum bet is your <span style={{ color:"var(--warn)", fontWeight:700 }}>bet spread</span>.
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[
                { tc:"TC ≤ 0",  bet:"1 unit",  color:"var(--danger)", note:"House has the edge — minimum bet" },
                { tc:"TC +1–2", bet:"2–3 units", color:"var(--warn)", note:"Slight player edge" },
                { tc:"TC +3–4", bet:"4–6 units", color:"var(--accent)", note:"Clear player advantage" },
                { tc:"TC +5+",  bet:"8–12 units", color:"var(--accent)", note:"Strong advantage — maximum bet" },
              ].map(r=>(
                <div key={r.tc} style={{ display:"flex", alignItems:"center", gap:12, background:"var(--bg-elev)", borderRadius:8, padding:"10px 14px" }}>
                  <div style={{ minWidth:72, fontSize:13, fontWeight:900, color:r.color }}>{r.tc}</div>
                  <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", minWidth:80 }}>{r.bet}</div>
          ...
          {/* Deviations */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>INDEX PLAYS (DEVIATIONS)</div>
            <div style={{ fontSize:14, color:"var(--text)", lineHeight:1.8, marginBottom:10 }}>
              When the true count is high or low enough, the mathematically correct play can differ from basic strategy. These departures are called <span style={{ color:"var(--danger)", fontWeight:700 }}>index plays</span>. The most valuable 18 are the <span style={{ color:"var(--warn)", fontWeight:700 }}>Illustrious 18</span>; the top 4 surrender plays are the <span style={{ color:"var(--warn)", fontWeight:700 }}>Fab 4</span>.
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {[
          ...
          {/* Learning Path */}
          <section>
            <div style={{ fontSize:11, letterSpacing:"0.25em", color:"var(--accent)", marginBottom:10 }}>LEARNING PATH</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                ...
                { step:"2", title:"Practice the Hi-Lo count",   desc:"Use the Count tab. Aim for 100% accuracy before adding speed.", color:"var(--accent)" },
                { step:"3", title:"Build speed",                desc:"Use the Speed tab. Work toward accurately counting a 10-card sequence.", color:"var(--warn)" },
      ...
      {mode==="Count" && (
        <div style={{ width:"100%", maxWidth:"min(720px,95vw)", minHeight:"calc(100vh - 200px)", display:"flex", flexDirection:"column", justifyContent:"center" }}>
          <div style={{ display:"flex", gap:14, marginBottom:16, marginTop:8, fontSize:"clamp(10px,1.4vw,16px)", justifyContent:"center" }}>
            <span style={{ color:"var(--text-3)" }}>CORRECT: <span style={{ color:"var(--accent)" }}>{countScore.correct}/{countScore.total}</span></span>
            {cAcc!==null && <span style={{ color:"var(--text-3)" }}>ACC: <span style={{ color:cAcc>=80?"var(--accent)":cAcc>=60?"var(--warn)":"var(--danger)" }}>{cAcc}%</span></span>}
            <span style={{ color:"var(--text-3)" }}>STREAK: <span style={{ color:"var(--warn)" }}>{streak}</span></span>
          </div>
          <div style={{ display:"flex", gap:5, marginBottom:18, justifyContent:"center" }}>
            {[{cards:"2–6",val:"+1",color:"var(--accent)"},{cards:"7–9",val:"0",color:"#777"},{cards:"10–A",val:"−1",color:"var(--danger)"}].map(r=>(
              <div key={r.cards} style={{ background:"var(--bg-elev)", border:`1px solid color-mix(in srgb, ${r.color} 13%, transparent)`, borderRadius:6, padding:"clamp(4px,1vh,10px) clamp(11px,2vw,22px)", textAlign:"center" }}>
                <div style={{ fontSize:"clamp(9px,1.1vw,13px)", color:"var(--text-3)", marginBottom:1 }}>{r.cards}</div>
          ...
          {!feedback ? (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
              <div style={{ fontSize:"clamp(11px,1.4vw,16px)", color:"var(--text-3)" }}>What is the running count?</div>
            ...
            <div style={{ textAlign:"center" }}>
              <div style={{
                ...
                background:feedback.correct?"color-mix(in srgb, var(--accent) 9%, transparent)":"color-mix(in srgb, var(--danger) 9%, transparent)",
                border:`1px solid ${feedback.correct?"color-mix(in srgb, var(--accent) 27%, transparent)":"color-mix(in srgb, var(--danger) 27%, transparent)"}`,
                color:feedback.correct?"var(--accent)":"var(--danger)", fontWeight:700, fontSize:16,
              }}>{feedback.message}</div>
              {!feedback.correct && (
                <div style={{ fontSize:13, color:"var(--text-3)", marginBottom:9 }}>
                  {cards.map((c,i)=>(
          ...
          {quizView==="quiz" && quizScenario && (
            <div style={{ textAlign:"center", paddingBottom:40 }}>
              <div style={{ display:"flex", gap:14, marginBottom:14, fontSize:14, justifyContent:"center" }}>
                <span style={{ color:"var(--text-3)" }}>CORRECT: <span style={{ color:"var(--accent)" }}>{quizScore.correct}/{quizScore.total}</span></span>
              ...
              {!quizFeedback ? (
                <div>
                  <div style={{ fontSize:14, color:"var(--text-3)", marginBottom:10 }}>What&apos;s the correct play?</div>
                    ...
                    {["H","S","D","P"].map(action=>{
                      ...
                      return (
                        <button key={action} onClick={()=>submitStrategy(action)} style={{
                          ...
                          background:st.bg, border:`1px solid color-mix(in srgb, ${st.border} 53%, transparent)`,
                ...
                <div>
                  <div style={{
                    padding:"11px 22px", borderRadius:9, marginBottom:12,
                    background:quizFeedback.correct?"color-mix(in srgb, var(--accent) 9%, transparent)":"color-mix(in srgb, var(--danger) 9%, transparent)",
                    border:`1px solid ${quizFeedback.correct?"color-mix(in srgb, var(--accent) 27%, transparent)":"color-mix(in srgb, var(--danger) 27%, transparent)"}`,
                    color:quizFeedback.correct?"var(--accent)":"var(--danger)", fontWeight:700, fontSize:16,
                  }}>
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap", justifyContent:"center", marginBottom:14 }}>
                {Object.entries(ACTION_STYLE).map(([k,v])=>(
                  <div key={k} style={{ display:"flex", alignItems:"center", gap:6, background:v.bg, border:`1px solid color-mix(in srgb, ${v.border} 27%, transparent)`, borderRadius:4, padding:"5px 12px" }}>
              ...
              {stratTab!=="deviations" && (
                <div style={{ background:"#090e0b", border:"1px solid var(--bg-elev-2)", borderRadius:10, padding:12, maxWidth:724, margin:"0 auto", width:"100%" }}>
                  ...
                  <div style={{ fontSize:12, color:"var(--text-3)", textAlign:"center", marginTop:8 }}>6-deck · Dealer stands soft 17 · DAS allowed</div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14, gap:10 }}>
                    ...
                    <button onClick={()=>{ setDevMode(d=>!d); if(!devMode) newDevQuiz(); }} style={{
                      padding:"8px 20px", borderRadius:5, border:`1px solid ${devMode?"var(--danger)":"color-mix(in srgb, var(--accent) 27%, transparent)"}`, cursor:"pointer",
                  ...
                  {/* Deviation Quiz */}
                  {devMode && devScenario && (
                    <div style={{ background:"var(--bg-elev)", border:"1px solid color-mix(in srgb, var(--danger) 20%, transparent)", borderRadius:12, padding:"16px", marginBottom:16, textAlign:"center" }}>
                      <div style={{ fontSize:9, color:"color-mix(in srgb, var(--danger) 53%, transparent)", letterSpacing:"0.2em", marginBottom:6 }}>DEVIATION QUIZ</div>
                      <div style={{ display:"flex", gap:12, marginBottom:12, fontSize:10, justifyContent:"center" }}>
                        <span style={{ color:"var(--text-3)" }}>CORRECT: <span style={{ color:"var(--accent)" }}>{devScore.correct}/{devScore.total}</span></span>
                        {dAcc!==null && <span style={{ color:"var(--text-3)" }}>ACC: <span style={{ color:dAcc>=80?"var(--accent)":dAcc>=60?"var(--warn)":"var(--danger)" }}>{dAcc}%</span></span>}
                      </div>
                      ...
                      {!devFeedback ? (
                        ...
                        <div>
                          <div style={{
                            ...
                            background:devFeedback.correct?"color-mix(in srgb, var(--accent) 9%, transparent)":"color-mix(in srgb, var(--danger) 9%, transparent)",
                            border:`1px solid ${devFeedback.correct?"color-mix(in srgb, var(--accent) 27%, transparent)":"color-mix(in srgb, var(--danger) 27%, transparent)"}`,
                            color:devFeedback.correct?"var(--accent)":"var(--danger)", fontWeight:700, fontSize:13,
                          }}>{devFeedback.correct?"Correct! 🎯":"Not quite"}</div>
                          {!devFeedback.correct && (
                            <div style={{ fontSize:10, color:"#888", marginBottom:10, lineHeight:1.7 }}>
                              <div>Action: <span style={{ color: devFeedback.correctAction?"var(--accent)":"var(--danger)" }}>{DEV_ACTION_STYLE[devScenario.dev]?.label}</span></div>
                              <div>TC threshold: <span style={{ color:"var(--warn)" }}>{devScenario.dir==="gte"?"≥":""}{devScenario.dir==="lte"?"≤":""}{devScenario.threshold}</span></div>
                            </div>
                          )}
                      ...
                      <table style={{ borderCollapse:"collapse", width:"100%", fontSize:13 }}>
                        <thead>
                          <tr style={{ background:"var(--bg-elev)" }}>
                            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"left", fontSize:12, letterSpacing:"0.1em", fontWeight:900 }}>HAND</th>
                            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"center", fontSize:12, letterSpacing:"0.05em", fontWeight:900 }}>BASE</th>
                            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"center", fontSize:12, letterSpacing:"0.05em", fontWeight:900 }}>TC</th>
                            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"center", fontSize:12, letterSpacing:"0.05em", fontWeight:900 }}>DEVIATE</th>
                            <th style={{ padding:"5px 8px", color:"var(--text-3)", textAlign:"center", fontSize:12, letterSpacing:"0.05em", fontWeight:900 }}>★</th>
                          </tr>
                        </thead>
                        <tbody>
                          {DEVIATIONS.map((d,i)=>{
                            ...
                            return (
                              <tr key={i} style={{ borderTop:"1px solid #0f1810", background:i%2===0?"#090e0b":"#0b1209" }}>
                                ...
                                <td style={{ padding:"5px 8px", textAlign:"center", fontWeight:900, fontSize:13 }}>
                                  <span style={{ color: d.dir==="gte"?"var(--accent)":"var(--danger)" }}>
                                    {dirSymbol}{d.tc}
                                  </span>
                                </td>
                                ...
                                <td style={{ padding:"5px 8px", textAlign:"center", fontSize:13, color:"var(--warn)" }}>{d.priority}</td>
                              </tr>
                            );
                          })}
                        </tbody>
      ...
      {mode==="Speed" && (
        <div style={{ width:"100%", maxWidth:"min(640px,95vw)", textAlign:"center", minHeight:"calc(100vh - 200px)", display:"flex", flexDirection:"column", justifyContent:"flex-start", paddingTop:24 }}>
          <div style={{ display:"flex", gap:14, marginBottom:14, fontSize:14, justifyContent:"center" }}>
            <span style={{ color:"var(--text-3)" }}>CORRECT: <span style={{ color:"var(--accent)" }}>{countScore.correct}/{countScore.total}</span></span>
            <span style={{ color:"var(--text-3)" }}>STREAK: <span style={{ color:"var(--warn)" }}>{streak}</span></span>
            <span style={{ color:"var(--text-3)" }}>BEST: <span style={{ color:"var(--warn)" }}>{bestStreak}</span></span>
          </div>
          <div style={{ fontSize:13, color:"var(--text-3)", marginBottom:10 }}>10 cards auto-flip. Track the count. Submit at the end.</div>
          ...
          <div style={{ display:"flex", gap:4, justifyContent:"center", marginBottom:18 }}>
            {speedCards.map((_,i)=>(
              <div key={i} style={{
                ...
                background:i<speedIndex?"var(--accent)":i===speedIndex?"var(--warn)":"var(--bg-elev-2)",
          ...
          {speedResult && (
            <div>
              <div style={{
                ...
                background:speedResult.correct?"color-mix(in srgb, var(--accent) 9%, transparent)":"color-mix(in srgb, var(--danger) 9%, transparent)",
                border:`1px solid ${speedResult.correct?"color-mix(in srgb, var(--accent) 27%, transparent)":"color-mix(in srgb, var(--danger) 27%, transparent)"}`,
                color:speedResult.correct?"var(--accent)":"var(--danger)", fontWeight:700,
              }}>
                {speedResult.correct?"Perfect! 🎯":`Count was ${speedResult.answer>0?"+":""}${speedResult.answer}`}
              </div>
              {!speedResult.correct && (
                <div style={{ fontSize:9, color:"var(--text-3)", marginBottom:9, lineHeight:1.9 }}>
                  {speedCards.map((c,i)=>(
        ...
        return (
          <div style={{width:'100%',maxWidth:'min(960px,95vw)',paddingTop:12}}>
            ...
            <div style={{display:'flex',gap:10,alignItems:'center',marginBottom:16,flexWrap:'wrap',justifyContent:'center'}}>
              <span style={{fontSize:13,color:'var(--text-3)'}}>PLAYERS:</span>
              {[1,2,3,4,5,6].map(n=>(
              ...
              {sim.hc>0&&<span style={{fontSize:12,color:'var(--text-3)'}}>HAND #{sim.hc}</span>}
            </div>
            ...
            <div style={{display:'flex',gap:14,alignItems:'flex-start'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{textAlign:'center',marginBottom:20}}>
                  <div style={{fontSize:11,letterSpacing:'0.2em',color:'var(--text-3)',marginBottom:8}}>DEALER</div>
                ...
                <div style={{display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap'}}>
                  {sim.seats.map((seat,i)=>{
                    ...
                    const bc=rc==='win'||rc==='BJ!'?'var(--accent)':rc==='push'?'var(--warn)':rc==='bust'||rc==='lose'?'var(--danger)':'var(--border)';
                    return(
                      <div key={i} style={{background:'var(--bg-elev)',border:`1px solid color-mix(in srgb, ${bc} 20%, transparent)`,borderRadius:10,padding:'10px 12px',textAlign:'center',minWidth:90}}>
                        <div style={{fontSize:11,color:'var(--text-3)',marginBottom:6}}>P{i+1}</div>
              ...
              {/* Count Panel */}
              <div style={{width:164,flexShrink:0,background:'var(--bg-elev)',border:'1px solid var(--border)',borderRadius:10,padding:14}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                  <span style={{fontSize:11,letterSpacing:'0.15em',color:'var(--text-3)'}}>COUNT</span>
                  <button onClick={()=>setSimReveal(r=>!r)} style={{
                    fontSize:10,padding:'3px 8px',borderRadius:4,border:`1px solid ${simReveal?'color-mix(in srgb, var(--accent) 27%, transparent)':'var(--border)'}`,
                    background:simReveal?'color-mix(in srgb, var(--accent) 13%, transparent)':'transparent',color:simReveal?'var(--accent)':'var(--text-3)',
                ...
                {simReveal?(
                  <>
                    <div style={{marginBottom:12}}>
                      <div style={{fontSize:11,color:'var(--text-3)',marginBottom:2}}>RUNNING</div>
                      <div style={{fontSize:30,fontWeight:900,color:sim.rc>0?'var(--accent)':sim.rc<0?'var(--danger)':'var(--text)'}}>
                        {sim.rc>0?'+':''}{sim.rc}
                      </div>
                    </div>
                    <div style={{marginBottom:12}}>
                      <div style={{fontSize:11,color:'var(--text-3)',marginBottom:2}}>DECKS LEFT</div>
                      <div style={{fontSize:22,fontWeight:900,color:'var(--text)'}}>{decks}</div>
                    </div>
                    <div style={{marginBottom:16}}>
                      <div style={{fontSize:11,color:'var(--text-3)',marginBottom:2}}>TRUE COUNT</div>
                      <div style={{fontSize:30,fontWeight:900,color:tc>0?'var(--accent)':tc<0?'var(--danger)':'var(--text)'}}>
                        {tc>0?'+':''}{tc}
                      </div>
                    </div>
                  </>
                ...
                <div>
                  ...
                  <div style={{background:'var(--bg)',borderRadius:4,height:8,overflow:'hidden'}}>
                    <div style={{height:'100%',borderRadius:4,background:sim.si>=234?'var(--danger)':'var(--accent)',width:`${(sim.si/312)*100}%`,transition:'width 0.3s'}} />
                  </div>
                  {sim.si>=234&&<div style={{fontSize:10,color:'var(--danger)',marginTop:4}}>RESHUFFLE NEXT HAND</div>}
                </div>
              </div>
            </div>
          </div>
